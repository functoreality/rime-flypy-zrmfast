# Rime 输入法双拼加辅助码方案
<!-- # 这是一个 Rime 配置文件示例 -->
Rime 输入法配置文件，小鹤双拼（也可设置为用自然码/微软双拼）+自然快手形码（也可设置为用小鹤/自然码形码）辅助方案。使用后打字几乎不需要翻页，且学习成本明显低于五笔等输入方案。
* 如果用户还不了解双拼双形输入方案，或者不熟悉 Rime 输入法软件，可以先查看
[双拼与 Rime 输入法入门](intro.md)
。当然，在那之前，您也可以先往下翻，看看本项目能提供什么样的功能。
* 如果用户只对本项目提供的辅助功能感兴趣，暂时不需要双拼输入，则可参考 [给全拼用户的说明](README_lunapy.md)。

## 方案说明
* 方案名称为“小鹤快手”，它提供了（默认）小鹤双拼+自然快手双形辅助选字 的输入方案，使用 `[` 引出辅助码。
	* 如果用户希望使用其他的双拼、双形方案，或修改辅助码的引导方式，可以参考下方 [方案偏好设置](#方案偏好设置) 部分的说明。
...
* 双拼方案默认为小鹤双拼。如果用户更习惯使用自然码/微软双拼，可在 `flypy_zrmfast.custom.yaml` 文件里设置“改用自然码双拼”/“改用微软双拼”（在文件中搜索这个引号里的内容，不包括引号本身，可以快速找到这个配置选项）。
	* 对于其他双拼方案的用户，可以考虑自行修改 `pinyin_switch.yaml` 中的拼写运算，将小鹤翻译成所需要的双拼（可以参照该文件中已经提供的翻译成自然码/微软双拼的运算）。
* 双形方案默认使用自然快手辅助码。如果用户更喜欢小鹤版本的双形辅助码，可以在 `flypy_zrmfast.custom.yaml` 文件里设置“使用小鹤原版辅助码”。
	* 在[这个页面](https://xh.flypy.com/#/xyx)的尾部可以看到小鹤的双形拆分规则。另外，本项目只提供单字码，对词组简码有需求的用户建议直接使用官方的小鹤音形输入法。
	* 注：如果当前项目中使用小鹤原版辅助码的码表涉及侵权，可联系项目作者删除。
	* 可以同理设置“使用自然码原版辅助码”。
* 双拼与双形部分默认使用 `[` 符号分隔，也就是用 `[` 键引导辅助码。
...
* 使用 `[` 引导辅助码时，如果实际未输入辅助码，可能在缓冲区中出现多余的 `[` 符号。
	* 例如想输入“辅”字，敲了 `fu[`，还没来得及继续敲 `i`，就发现“辅”字已经出现在了候选列表中，但此时直接选择会导致输入了一个多余的 `[` 符号。
	* 针对这种情况，可以在 `flypy_zrmfast.custom.yaml` 文件里设置“无视多余的引导符”，从而让 `fu[` 这整个输入码直接对应“辅”这个字，`[` 不再多余。
		* （该解决方案来自 [GitHub issue](https://github.com/functoreality/rime-flypy-zrmfast/issues/7#issuecomment-2056552302)）
	* 不过这种方案可能导致双拼双形含义混淆的问题，例如想输入“厦”这个字，其编码为 `ua[ie`，而直接敲 `ua[ie` 所得的第一候选可能是“刹车”这个词组。因此，用户在进行这项设置前，需要考虑这一副作用是否可以接受。
...
* `ae` 前缀：emoji 模式，注意这里需要按照英文输入。例如：敲 `aelaugh` 空格，即可输入“😂”。
	* 如果用户希望直接在汉字输入候选栏推荐 emoji 表情，例如敲 `wwxn` 后在“微笑”候选之后看到“😊”，可按照 `flypy_zrmfast.custom.yaml` 里的说明设置“汉字候选区推荐表情符号”。
...
* `/` 前缀：符号模式，具体见 Rime 的系统目录自带的 `symbols.yaml` 文件。
	...
	* 如果用户希望直接在汉字输入候选栏推荐符号，例如敲 `pkfh` 后在“平方”候选之后看到“²”，可按照 `flypy_zrmfast.custom.yaml` 里的说明设置“汉字候选区推荐表情符号”。
